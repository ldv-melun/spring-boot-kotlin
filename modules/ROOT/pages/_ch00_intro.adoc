...
En environnement de développement, les développeurs disposent d'un `internal web server`, un serveur http intégré à leur IDE, https://tomcat.apache.org/[Apache Tomcat], utilisé pour les tests en phase de développement.
...
Le moyen le plus simple pour générer une application Spring Boot est de passer par le site https://start.spring.io qui permet positionner un certain nombre de paramètres et de choisir les dépendances à utiliser (présenté dans prochainement)
